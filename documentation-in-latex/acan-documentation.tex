...
\begin{center}
  \begin{tabular}{lll}
    ...
    1.0.0 & October xxx, 2017 & Initial release \\
  \end{tabular}
\end{center}
...
This error is raised when you have changed the CAN bit properties (\texttt{mBitRatePrescaler}, \texttt{mPropagationSegment}, \texttt{mPhaseSegment1}, \texttt{mPhaseSegment2}, \texttt{mRJW}), and one or more resulting values are inconsistent. See \refSubsectionPage{checkCANBitSettingConsistency}.
...
The second argument does not change the CAN bit computation, it only changes the acceptance test. For example, you can specify that you want the computed actual bit to be exactly the wished bit rate:
{ \small\begin{lstlisting}[language=c++]
void setup () {
  Serial.begin (9600) ;
...
void setup () {
  ...
  Serial.println (settings.checkCANBitSettingConsistency ()) ; // 0, meaning Ok
  ...
}
\end{lstlisting}}
...
Note the computation may calculate a bit decomposition too far from the wished bit rate, but it is always consistent. You can check this by calling the \texttt{checkCANBitSettingConsistency} method.
...
\subsectionLabel{The \texttt{checkCANBitSettingConsistency} method}{checkCANBitSettingConsistency}
...
void setup () {
  ...
  Serial.println (settings.checkCANBitSettingConsistency (), HEX) ; // 0x10, meaning error
  ...
}
\end{lstlisting}}

The \texttt{checkCANBitSettingConsistency} method returns $0$ if CAN bit decomposition is consistent. Otherwise, the returned value is a bit field that can report several errors -- see \refTableau{checkCANBitSettingConsistencyErrorCode}.
...
\begin{table}[htbp]
  ...
  \end{tabular}
  \caption{The \texttt{ACANSettings::checkCANBitSettingConsistency} method error codes}
  \labelTableau{checkCANBitSettingConsistencyErrorCode}
\end{table}
...
{\bf Note. } If CAN bit settings are not consistent (see \refSubsectionPage{checkCANBitSettingConsistency}), the returned value is irrelevant.
