...
The \texttt{ACAN} library is a CAN ("Controller Area Network") driver for Teensy 3.x. It has been designed to make it easy to start and to be easily configurable:
\begin{itemize}
...
{\bf A non conform filter never matches.}
...
\sectionLabel{The \texttt{begin} method reference}{beginMethodReference}

\subsection{The \texttt{begin} method prototype}
...
\begin{table}[htbp]
  ...
  \begin{tabular}{llllllllll}
    \multicolumn{8}{c}{\textbf{Error Codes}} & \textbf{Comment} & \textbf{Link}\\
    31 -- 7 & 6 & 5 & 4 & 3 & 2 & 1 & 0 & & \\
    \texttt{0} ...  \texttt{0} &\texttt{0} &\texttt{0} &\texttt{0} &\texttt{0} &\texttt{0} &\texttt{0} &\texttt{0} & No error & \\
    \texttt{0} ...  \texttt{0} &\texttt{0} &\texttt{0} &\texttt{0} &\texttt{0} &\texttt{0} &\texttt{0} &\texttt{1} & CAN Bit setting configuration error & \refSubsectionPage{CANBitConfigError}\\
    \texttt{0} ...  \texttt{0} &\texttt{x}&\texttt{x} &\texttt{x} &\texttt{x} &\texttt{x} &\texttt{1} &\texttt{0} & Too much primary filters & \refSubsectionPage{tooMuchPrimaryFiltersError}\\
    \texttt{0} ...  \texttt{0} &\texttt{x} &\texttt{x} &\texttt{x} &\texttt{x} &\texttt{1} &\texttt{x} &\texttt{0} & Primary filter conformance error &  \refSubsectionPage{primaryFilterConformanceError}\\
    \texttt{0} ...  \texttt{0} &\texttt{x}&\texttt{x} &\texttt{x} &\texttt{1} &\texttt{x} &\texttt{x} &\texttt{0} & Too much secondary filters & \refSubsectionPage{tooMuchSecondaryFiltersError}\\
    \texttt{0} ...  \texttt{0} &\texttt{x}&\texttt{x} &\texttt{1} &\texttt{x} &\texttt{x} &\texttt{x} &\texttt{0} & Secondary filter conformance error &  \refSubsectionPage{secondaryFilterConformanceError}\\
    \texttt{0} ...  \texttt{0} &\texttt{x}&\texttt{1} &\texttt{x} &\texttt{x} &\texttt{x} &\texttt{x} &\texttt{0} & \texttt{ACAN::can1} has no Tx alternate pin & \refSubsectionPage{noAlternateTxPinError}\\
    \texttt{0} ...  \texttt{0} &\texttt{1}&\texttt{x} &\texttt{x} &\texttt{x} &\texttt{x} &\texttt{x} &\texttt{0} & \texttt{ACAN::can1} has no Rx alternate pin & \refSubsectionPage{noAlternateRxPinError} \\
  \end{tabular}
  \caption{The \texttt{begin} method error codes}
...
\subsectionLabel{CAN Bit setting configuration error}{CANBitConfigError}

This error is raised when the \texttt{mBitSettingOk} of the \texttt{settings} object is false. This means that the \texttt{ACANSettings} constructor cannot compute the CAN bit setting for the given bit rate. When the \texttt{begin} is called with \texttt{settings.mBitSettingOk} false, this error is reported. For example:
...
This error is a fatal error, the driver and the FlexCAN module are not configured. See \refSectionPage{CANbitSettings} for a discussion about CAN bit setting computation.

\subsectionLabel{Too much primary filters error}{tooMuchPrimaryFiltersError}
...
\subsectionLabel{Too much secondary filters error}{tooMuchSecondaryFiltersError}
...
\subsectionLabel{Secondary filter conformance error}{secondaryFilterConformanceError}
...
\subsectionLabel{No alternate Tx pin error}{noAlternateTxPinError}

In the Teensy 3.6, \texttt{ACAN::can1} does not support alternate Tx pin.


\subsectionLabel{No alternate Rx pin error}{noAlternateRxPinError}

In the Teensy 3.6, \texttt{ACAN::can1} does not support alternate Rx pin.



\sectionLabel{Computing the CAN bit settings}{CANbitSettings}

The constructor of the \texttt{ACANSettings} has one mandatory argument: the wished bit rate. It tries to compute the CAN bit settings for this bit rate. If it succeeds, the constructed object has its \texttt{mBitSettingOk} property set to \texttt{true}, otherwise it is set to \texttt{false}. For example:
{ \small\begin{lstlisting}[language=c++]
void setup () {
...
The fastest exact bit rate is 3,2 Mbit/s. It works when the FlexCAN module is configured in both \emph{loop back} mode (\refSubsectionPage{mLoopBackMode}) and \emph{self reception} mode (\refSubsectionPage{mSelfReceptionMode}). Note bit rates above 1 Mbit/s do not conform to the \texttt{ISO-11898}; CAN transceivers as \texttt{MCP2551} require the bit rate lower or equal to 1 Mbit/s.
...
\begin{align*}
1 & \leqslant BRP \leqslant 256 \\
1 & \leqslant RJW \leqslant 4 \\
1 & \leqslant PropSeg \leqslant 8 \\
\text{Single sampling: }1 & \leqslant PS_1 \leqslant 8\\
\text{Triple sampling: }2 & \leqslant PS_1 \leqslant 8\\
2 & \leqslant PS_2 \leqslant 8 \\
RJW &\leqslant PS_2
\end{align*}

And resulting bit rate and sampling point (in per-cent unit) are given by:
\begin{align*}
\text{Actual bit rate} & = \frac{16~\text{MHz}}{BRP \cdot (1 + PropSeg + PS_1 +PS_2)} \\
\text{Sampling point {\it(single sampling)}} & = \frac{1 + PropSeg + PS_1}{1 + PropSeg + PS_1 +PS_2} \cdot 100 \\
\text{Sampling first point {\it(triple sampling)}} & = \frac{PropSeg + PS_1}{1 + PropSeg + PS_1 +PS_2} \cdot 100
\end{align*}
...
\sectionLabel{Properties of the \texttt{ACANSettings} class}{propertiesMoliCanSettings}

All properties of the \texttt{ACANSettings} class are declared \texttt{public} and are initialized. The default values of properties from \texttt{mWhishedBitRate} until \texttt{mTripleSampling} corresponds to a CAN bit rate of 250,000 bit/s.
...
\begin{table}[htbp]
  ...
  \begin{tabular}{lllll}
    ...
    \texttt{mWhishedBitRate} & \texttt{uint32\_t} & \texttt{250,000} & see \refSectionPage{CANbitSettings}\\
    \texttt{mBitRatePrescaler} & \texttt{uint16\_t} & \texttt{4} & see \refSectionPage{CANbitSettings} (valid values: $1$ ... $256$)\\
    \texttt{mPropagationSegment} & \texttt{uint8\_t} & \texttt{5} & see \refSectionPage{CANbitSettings} (valid values: $1$ ... $8$)\\
    \texttt{mPhaseSegment1} & \texttt{uint8\_t} & \texttt{5} & see \refSectionPage{CANbitSettings} (valid values: $1$ ... $8$)\\
    \texttt{mPhaseSegment2} & \texttt{uint8\_t} & \texttt{5} & see \refSectionPage{CANbitSettings} (valid values: $2$ ... $8$)\\
    \texttt{mRJW} & \texttt{uint8\_t} & \texttt{4} & see \refSectionPage{CANbitSettings} (valid values: $1$ ... $4$)\\
    \texttt{mTripleSampling} & \texttt{bool} & \texttt{false} & see \refSectionPage{CANbitSettings} \\
    \texttt{mBitSettingOk} & \texttt{bool} & \texttt{true} & see \refSectionPage{CANbitSettings} \\
    \texttt{mListenOnlyMode} & \texttt{bool} & \texttt{false} & see \refSubsectionPage{mListenOnlyMode} \\
    \texttt{mSelfReceptionMode} & \texttt{bool} & \texttt{false} & see \refSubsectionPage{mSelfReceptionMode} \\
    \texttt{mLoopBackMode} & \texttt{bool} & \texttt{false} & see \refSubsectionPage{mLoopBackMode} \\
    ...
    \texttt{mMessageIRQPriority} & \texttt{uint8\_t} & \texttt{64} & see \refSubsectionPage{mMessageIRQPriority}\\
  ...
  \labelTableau{tablePropertiesMoliCanSettings}
\end{table}


\subsectionLabel{The \texttt{mListenOnlyMode} property}{mListenOnlyMode}

This bit corresponds to the \texttt{LOM} bit of the FlexCAN \texttt{CTRL1} control register.



\subsectionLabel{The \texttt{mSelfReceptionMode} property}{mSelfReceptionMode}

This bit corresponds to the complement of the \texttt{SRXDIS} bit of the FlexCAN \texttt{MCR} control register.



\subsectionLabel{The \texttt{mLoopBackMode} property}{mLoopBackMode}

This bit corresponds to the \texttt{LBP} bit of the FlexCAN \texttt{CTRL1} control register.





\subsectionLabel{The \texttt{mMessageIRQPriority} property}{mMessageIRQPriority}
